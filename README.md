...
- 🎯 Entry points to execute your program using `python -m <flask_template_trial>` or `$ flask_template_trial` with basic CLI argument parsing.
...
---
# flask_template_trial Flask Application

Awesome flask_template_trial created by e-simons
...
```bash
git clone https://github.com/e-simons/flask-template-trial flask_template_trial
cd flask_template_trial
make install
```
...
```bash
pip install flask_template_trial
```
...
```bash
$ flask_template_trial
```

or

```bash
$ python -m flask_template_trial
```
...
```bash
flask_template_trial create-db   # run once
flask_template_trial populate-db  # run once (optional)
flask_template_trial add-user -u admin -p 1234  # ads a user
flask_template_trial run
```
