...
original_author="e-simons"
original_name="flask_template_trial"
original_urlname="flask-template-trial"
original_description="Awesome flask_template_trial created by e-simons" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_template_trial "${name}"
...
then
    ...
    echo "then run 'flask_template_trial' or 'python -m flask_template_trial'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_template_trial' or 'python -m flask_template_trial' or 'poetry run flask_template_trial'"
fi
